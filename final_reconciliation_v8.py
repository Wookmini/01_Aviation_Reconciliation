...
output_file = 'excel_workspace/항공권 정산/항공권 결제내역 정산(04월분)_최종_정밀완성_v8.xlsx'
...
try:
    print("v8 정밀 정산 및 시트 정제 작업을 시작합니다...")
    ...
    for row in range(2, ws_sejung.max_row + 1):
        ...
        is_matched_at_all = (ac_val in dahs_approval_numbers or av_val in dahs_approval_numbers)
        
        # 데이터가 있는 행인지 체크 (이름이나 금액이 있어야 함)
        has_data = ws_sejung.cell(row=row, column=12).value or ah_amt != 0 or at_amt != 0
        if not has_data: continue

        if not is_matched_at_all:
            # 아예 없으면 노란색
    ...
    wb.save(output_file)
    print(f"\n[v8 업데이트 완료]")
    print(f"- [영문국문명] 시트 B열 공백 제거 완료")
    print("- 세중 31~49행 제외 로직 적용")
